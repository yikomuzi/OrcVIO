...
void YoloObjectDetector::cameraCallback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  try {
    if (msg->encoding == "bgr8" || msg->encoding == "rgb8") {
      ...
      ROS_WARN_ONCE("Converting mono8 images to bgr8");
      cam_image = cv_bridge::toCvCopy(msg, "bgr8");
    } else {
...
void* YoloObjectDetector::fetchInThread() {
  ...
  // if (buff_[buffIndex_].c == 3) rgbgr_image(buff_[buffIndex_]);
...
void* YoloObjectDetector::publishInThread() {
  if (lastImageHeaderStamp_ == imageHeader_.stamp) {
      return 0;
  }
  // Publish image.
  ...
  lastImageHeaderStamp_ = imageHeader_.stamp;
  for (int i = 0; i < numClasses_; i++) {
    rosBoxes_[i].clear();
    rosBoxCounter_[i] = 0;
  }
